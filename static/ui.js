...
function renderModelDropdown(){
  ...
  // ── Configured-model badge data (#1539) ──────────────────────────────────
  // Walk the underlying <select> options to pair each model with its
  // configured-model badge (primary / fallback N / configured) from
  // window._configuredModelBadges. Models referenced by the configured
  // chain but missing from the dropdown catalog are synthesized so they
  // still appear under the "Configured" header.
  const _modelData=[];
  ...
  const _badgeByValue=new Map(_modelData.filter(m=>m.badge).map(m=>[m.value,{name:m.name,id:m.id,badge:m.badge}]));
  ...
  // Prepend a virtual "Configured" section that pins primary + fallback
  // models at the top of the picker (closest to upstream's behavior).
  const _configuredEntries=Array.from(_badgeByValue.entries())
    .map(([value,info])=>({value,name:info.name,id:info.id,badge:info.badge}))
    .sort((a,b)=>{
      const ra=_configuredRank(a.badge), rb=_configuredRank(b.badge);
      if(ra!==rb) return ra-rb;
      return a.name.localeCompare(b.name);
    });
  if(_configuredEntries.length){
    sections.unshift({
      id:'__configured__',
      displayName:t('model_group_configured')||'Configured',
      models:_configuredEntries,
      logoMeta:{display_name:'Configured',id:'__configured__'},
      isVendo:false,
      isConfigured:true,
    });
  }
  ...
  const _renderSections=(term)=>{
    ...
    for(const section of sections){
      ...
      for(const m of matches){
        ...
        // Pull configured-model badge data when present (#1539). The
        // virtual "Configured" section already carries badges on its
        // model entries; per-provider sections look them up by value.
        const badgeInfo=m.badge||(_badgeByValue.get(m.value)||{}).badge||null;
        let badgeHtml='';
        if(badgeInfo){
          let badgeLabel=badgeInfo.label||'Configured';
          if(badgeInfo.provider){
            const providerName=String(badgeInfo.provider).replace(/^custom:/,'').split('/')[0];
            badgeLabel+=` (${providerName})`;
          }
          badgeHtml=`<span class="model-opt-badge model-opt-badge--${esc(badgeInfo.role||'configured')}">${esc(badgeLabel)}</span>`;
        }
        row.innerHTML=`<div class="model-opt-top"><span class="model-opt-name">${esc(m.name||'')}</span>${badgeHtml}</div><span class="model-opt-id">${esc(m.id||'')}</span>`;
